...
public interface SentenceMapper extends BaseMapper<Sentence> {
    ...
    @Select("SELECT s.id, r.id AS recordId, s.word_id, s.sentence, (r.id IS NOT NULL) AS isRecord, (SELECT find_in_set('', temp_user.type) FROM user temp_user WHERE temp_user.id = #{userId}) AS isEdit " +
            "FROM sentence s " +
